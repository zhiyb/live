...
ffmpeg -hide_banner -hwaccel qsv -hwaccel_output_format qsv \
    ...
    \
    -filter:v:4 scale_qsv=format=nv12:w=1920:h=1080,vpp_qsv=tonemap=1 \
    -c:v:4 h264_qsv -b:v:4 8M -profile:v:4 baseline $qsv_h264_params \
    \
    -filter:v:5 scale_qsv=format=nv12:w=1280:h=720,vpp_qsv=tonemap=1 \
    -c:v:5 h264_qsv -b:v:5 3200k -profile:v:5 baseline $qsv_h264_params \
    \
    -filter:v:6 scale_qsv=format=nv12:w=854:h=480,vpp_qsv=tonemap=1 \
